...
Do not fix the visible consumer until the first incorrect boundary is proven. Do not add fallback context that hides an upstream loss. Preserve response contracts and existing successful flows. Add or update the narrowest test that reproduces the failed transition, then load safe-edit-verify.
