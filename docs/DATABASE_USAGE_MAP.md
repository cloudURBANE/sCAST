...
|---|---|---|---|---|---|---|---|---|
| `public.users` | table | `routes/auth.ts`, `routes/oauth.ts`, `routes/wardrobe.ts`, `routes/share.ts`, `routes/debug.ts`; `lib/db/src/schema/users.ts` | Same schema | 4 rows | `id uuid`, `email text`, `token uuid`, `oauth_provider text`, `oauth_subject text`, `created_at timestamp` | Login, token auth, share handles, wardrobe owner lookup | Yes | High |
| `public.user_fragrances` | table | `routes/wardrobe.ts`, `routes/share.ts`, `routes/debug.ts`; `lib/db/src/schema/userFragrances.ts` | Same schema | 23 rows | `id uuid`, `user_id uuid`, `fragrance_data jsonb`, `created_at timestamp` | Dashboard wardrobe, share page, rebuild, visibility, delete | After login for app boot | High |
...
| `public.global_fragrances` | table | `services/catalogService.ts`, `services/imageHydration.ts`, `routes/debug.ts`; `lib/db/src/schema/globalFragrances.ts` | Same schema | 36 rows | `id uuid`, `lookup_key text`, `name text`, `brand text`, `profile_data jsonb`, `created_at`, `updated_at` | Search cache, image refresh, rebuild, hydration | No | High |
