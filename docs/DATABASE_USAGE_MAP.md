...
|---|---|---|---|---|---|---|---|---|
| `public.users` | table | `routes/auth.ts`, `routes/oauth.ts`, `routes/wardrobe.ts`, `routes/share.ts`; `lib/db/src/schema/users.ts` | Same schema | 4 rows | `id uuid`, `email text`, `token uuid`, `oauth_provider text`, `oauth_subject text`, `created_at timestamp` | Login, token auth, share handles, wardrobe owner lookup | Yes | High |
