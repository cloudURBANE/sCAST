...
>
> 1. **Model credentials — OpenRouter.** Beam routes its reasoning model through
>    **OpenRouter** (one `OPENROUTER_API_KEY`, pay-per-token, many models), which is
>    the **same provider the production in-process loop uses**. This is the default
>    path here. (Anthropic-direct still works if you'd rather pin Claude without
>    OpenRouter — set `provider: anthropic` and `ANTHROPIC_API_KEY`; note the
>    Anthropic **OAuth** path needs Claude **Max + extra credits** and is owner-only,
>    not a basis for serving customers.) See [`providers`](https://hermes-agent.nousresearch.com/docs/integrations/providers).
...
## 2. Pin the reasoning model via OpenRouter

```powershell
hermes model
```

Choose **OpenRouter** as the provider and set the key in `~/.hermes/.env`:

```
OPENROUTER_API_KEY=sk-or-...
```

Then pick a tool-calling model slug (confirm the exact alias your Hermes build
expects — some want `openrouter/<vendor>/<model>`):

- **Strong tier** (synthesis / "build me a collection"): e.g. `anthropic/claude-opus-4.8`.
- **Cheap tier** (routine chat / routing): e.g. `anthropic/claude-haiku-4.5`.

You are not limited to Claude through OpenRouter — any tool-calling model works.
(To pin Claude directly instead, choose **Anthropic** here and set `ANTHROPIC_API_KEY`.)

Confirm a plain chat works before wiring Beam (Hermes needs ≥64K context — the
models above qualify):
...
```yaml
provider: openrouter                 # Beam routes through OpenRouter (see step 2)
model: "anthropic/claude-opus-4.8"   # OpenRouter slug; confirm via `hermes model`
