...
- graceful failure when `ANTHROPIC_API_KEY` is absent.

**To turn on:** add the one-line mount + set `ANTHROPIC_API_KEY` (see the module
README), after running `pnpm --filter @workspace/api-server run typecheck`.
