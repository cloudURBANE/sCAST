...
| Login | Same as signup for Google; legacy email endpoint exists | `routes/oauth.ts`; legacy `POST /api/auth/login` in `routes/auth.ts` | Same as signup; legacy endpoint select/insert by email (requires `ADMIN_SECRET`) | None | `public.users` | Same as signup | Existing rows required to preserve old token/session continuity | Same as signup; `ADMIN_SECRET` for legacy endpoint | New DB without old `public.users` creates new user/token, making old wardrobe ownership appear gone |
