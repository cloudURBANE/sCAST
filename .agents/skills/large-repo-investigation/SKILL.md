...
1. Use $repo-navigation to locate the route or page entry.
...
4. For visual or responsive issues, use $visual-ui-debug.
5. For conversation, memory, context, or agent-response issues, use $state-agent-debug.
...
8. Use $safe-edit-verify to apply the smallest safe change and run proportional checks.
9. Use $commit-discipline when a commit is in scope.
