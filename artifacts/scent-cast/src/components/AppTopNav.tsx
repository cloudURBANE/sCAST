  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="start"
        sideOffset={10}
        className={`w-56 rounded-[8px] border-scent-accent/25 bg-[#090604]/95 p-1.5 text-foreground shadow-[0_18px_48px_rgba(0,0,0,0.62)]${touchPerformanceMode ? '' : ' backdrop-blur-sm'}`}
      >
        {displayName ? (
          <>
          ...
          {navItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  [
                    'inline-flex min-h-11 flex-col items-center justify-center gap-px rounded-full px-2 text-[9.5px] font-semibold uppercase tracking-[0.08em] transition-colors duration-200 active:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-scent-accent/55',
                    // The filled pill stays the selected state, but desaturated
                    // toward a muted champagne (was #C6A94F→#A5852E, and brighter
                    // golds before that) so an already-active Home tab reads as a
                    // quiet "you are here" marker rather than the loudest CTA on
                    // the page — the forecast hero, not the nav, wins.
                    isActive
                      ? 'bg-gradient-to-b from-[#BCA45E] to-[#93793B] text-[#1a1206] shadow-[inset_0_1px_0_rgba(255,247,236,0.24)]'
