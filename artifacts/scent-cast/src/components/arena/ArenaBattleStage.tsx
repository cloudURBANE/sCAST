...
import { arenaPercentFor } from '@/components/arena/arenaTwists';
...
export const ArenaBattleStage: React.FC<ArenaBattleStageProps> = ({
  ...
  return (
    <section aria-labelledby="arena-battle-title" className="relative">
      <header className="mx-auto max-w-4xl px-2 text-center">
        <p className="scent-type-label text-scent-accent">ScentBeam Arena</p>
        <h1 id="arena-battle-title" className="mt-3 text-balance font-serif text-4xl italic leading-[0.98] text-[#fff7ec] sm:text-6xl">
          {battle.title}
        </h1>
      ...
      <div className="relative mx-auto mt-8 grid w-full max-w-[1320px] gap-4 lg:grid-cols-[minmax(0,1fr)_5.5rem_minmax(0,1fr)] lg:items-stretch">
        <ArenaBattleSide
          side={battle.left}
          align="left"
        ...
        <div className="pointer-events-none absolute left-1/2 top-[calc(50%-1.75rem)] z-20 hidden -translate-x-1/2 lg:block" aria-hidden="true">
          <div className="grid h-16 w-16 place-items-center rounded-full border border-scent-accent/48 bg-black/88 font-serif text-2xl italic text-scent-accent shadow-[0_0_34px_rgba(212,175,55,0.18)]">
            VS
          </div>
        </div>
        <div className="grid place-items-center lg:relative">
          <div className="grid h-14 w-14 place-items-center rounded-full border border-scent-accent/44 bg-black/82 font-serif text-xl italic text-scent-accent shadow-[0_0_28px_rgba(212,175,55,0.16)] lg:hidden" aria-hidden="true">
            VS
          </div>
        </div>
      ...
      {guestLocalOnly ? (
        <div className="mx-auto mt-5 flex max-w-2xl flex-col items-center gap-3 rounded-[var(--radius-scent)] border border-scent-accent/18 bg-black/54 px-4 py-4 text-center sm:flex-row sm:justify-between sm:text-left">
