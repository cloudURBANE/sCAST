  ...
  >
    ...
    <div className="relative z-10 flex min-h-[clamp(18rem,46svh,26rem)] flex-col sm:min-h-[clamp(24rem,45svh,34rem)]">
      <div className="mb-2 flex items-center justify-between gap-2 sm:mb-4 sm:gap-4">
        <span className="scent-type-label text-[10px] tracking-[0.1em] text-scent-accent/78 sm:text-[13px] sm:tracking-[0.14em]">{align === 'left' ? 'Option A' : 'Option B'}</span>
        {selected ? (
      ...
      <div className="relative flex flex-1 min-h-[9.75rem] items-end justify-center overflow-hidden rounded-[calc(var(--radius-scent)-10px)] border border-scent-accent/10 bg-black/[0.18] sm:min-h-[15rem] sm:rounded-[calc(var(--radius-scent)-6px)]">
        <div className="absolute inset-x-4 bottom-5 h-px bg-gradient-to-r from-transparent via-scent-accent/28 to-transparent sm:inset-x-8 sm:bottom-8" aria-hidden="true" />
        <BottleImage
          src={side.imageUrl}
      ...
      <div className="mt-3 min-w-0 text-center sm:mt-5">
        {side.brand ? (
          <BrandGoldLabel as="p" brand={side.brand} className="scent-card-brand scent-arena-brand mx-auto block max-w-full" />
        ) : (
      ...
      <button
        ...
        disabled={disabled}
        className={[
          'scent-primary-button scent-no-mobile-focus-ring mt-3 inline-flex min-h-11 w-full items-center justify-center gap-1.5 rounded-[calc(var(--radius-scent)-6px)] px-2 py-2 text-[11px] font-bold uppercase tracking-[0.1em] transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-scent-accent/70 disabled:pointer-events-none disabled:opacity-60 sm:mt-5 sm:min-h-14 sm:gap-2 sm:rounded-[var(--radius-scent)] sm:px-5 sm:py-3 sm:text-sm sm:tracking-[0.16em]',
          selected
